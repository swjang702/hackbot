...
| `docs/refs/LinnOS_summary.md` | LinnOS (OSDI 2020) paper summary — in-kernel NN for I/O latency prediction. Key prior art for Layer 1 reflex classifiers. |
| `docs/refs/HyperAgents_summary.md` | HyperAgents (Meta 2026) paper summary — self-referential self-improving agents. Key reference for Layer 3 self-improvement. |
| `docs/refs/hackbot_vision_synthesis.md` | Vision synthesis — 5-layer autonomic OS architecture (Sensory→Reflex→Reasoning→Self-Improvement→Safety). |
...
| `hackbot-kmod/hackbot_patrol.c` | Autonomous patrol kthread (C) — `kthread_run("hackbot_patrol")`, wakes every 120s, calls Rust `hackbot_patrol_tick()`. |
| `hackbot-kmod/hackbot_patrol.h` | C header for patrol kthread — `hackbot_patrol_start/stop`. |
| `hackbot-kmod/hackbot_memory.rs` | Agent memory ring buffer (Rust) — 8 entries × 512B, records patrol + user findings, injects into vLLM system prompt. |
| `hackbot-kmod/hackbot_trace.c` | Continuous tracepoint sensing (C) — three-tier data (raw ring + LinnOS features + aggregates) for sched_switch, sys_enter, block_rq_complete. Always-on from module load. |
| `hackbot-kmod/hackbot_trace.h` | C header for trace sensing — `hackbot_trace_init/exit/read_*/reset`. |
| `hackbot-kmod/hackbot_tokenizer.c` | Semantic kernel-event tokenizer (C) — maps raw tracepoint events into a compact vocabulary for downstream language modelling. |
| `hackbot-kmod/hackbot_tokenizer.h` | C header for the kernel-event tokenizer. |
| `hackbot-kmod/hackbot_ngram.c` | In-kernel n-gram learner + anomaly detector (C) — online updates over the tokenizer output, perplexity-based anomaly flagging. |
| `hackbot-kmod/hackbot_ngram.h` | C header for the n-gram learner. |
| `hackbot-kmod/kernel_version.rs` | Auto-generated — kernel release string constant (`"6.19.8"`), generated by Kbuild. |
| `hackbot-kmod/Kbuild` | Kernel build config — mixed C+Rust compilation, sanitizer opt-ins (KASAN/UBSAN/KCSAN), `-Wframe-larger-than=1024`, FPU flags for `hackbot_fpu.o`. |
| `hackbot-kmod/Makefile` | Build harness — `default`, `clean`, `load`, `unload`, `test`, `test-full`, `test-tools`, `test-patrol`, `check` (sparse), `check-warn` (W=1), `check-all`, `check-kconfig`. |
| `hackbot-kmod/test.sh` | Integration test suite — per-feature tests (lifecycle, I/O, 7 tools, patrol, memory) with real-time dmesg output. |
| `hackbot-kmod/TESTING.md` | Testing guide — manual + automated testing for all features, dmesg reference, architecture overview. |
