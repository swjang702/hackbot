...
| `hackbot-kmod/Kbuild` | Kernel build config — mixed C+Rust compilation, 7 C object files + Rust, FPU flags for `hackbot_fpu.o`. |
| `hackbot-kmod/Makefile` | Build harness — targets: `default`, `clean`, `load`, `unload`, `test`, `test-full`, `test-tools`, `test-patrol`. |
