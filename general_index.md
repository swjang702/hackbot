...
| `hackbot-kmod/hackbot_tools.rs` | Kernel observation tools (read-only) — `ps` (process listing via RCU walk), `mem` (memory stats via `si_meminfo`), `loadavg`. Parses `<tool>NAME</tool>` tags from LLM output. |
...
| `hackbot-kmod/hackbot_fpu.c` | Float32 FPU forward pass (C) — SmolLM2-135M transformer with FP16 weights, float32 activations, `kernel_fpu_begin/end` guards, software FP16→F32 conversion, fast math approximations. |
