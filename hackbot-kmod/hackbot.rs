...
/// Binary format version. Must match between exporter and loader.
...
fn parse_model_header(data: &[u8]) -> Result<ModelConfig> {
    ...
    if version != MODEL_FORMAT_VERSION {
        pr_err!("hackbot: unsupported model version: {} (expected {})\n",
    ...
    // Verify group_size divides all weight column dimensions
...
fn parse_and_store_model(data: &[u8], slot: &mut ModelSlot) -> Result {
    ...
    for l in 0..n_layers {
        ...
        // Helper closure to handle errors with cleanup
    ...
    if cursor != data.len() {
        ...
        // Not fatal — just warn. The extra bytes might be padding or future extensions.
    }

    pr_info!("hackbot: weights parsed: {} bytes ({} layers)\n",
             cursor - pos, n_layers);
    ...
    // data_addr and data_len are set by the caller
    // loaded is set by the caller after this returns Ok
...
fn free_model_resources() {
    ...
    // Free the inference state buffer (KV cache + activations)
...
fn encode_bpe(slot: &ModelSlot, input: &[u8], out_tokens: &mut [u32]) -> usize {
    ...
    // Preprocess: use the logits buffer as scratch for GPT-2 encoded input.
    // logits buffer is [i32; vocab_size] = 196 KB, we use up to MAX_PREPROC_INPUT bytes.
    // SAFETY: inf_buf_addr is valid, inf_logits offset is within bounds.
    // Logits buffer is not needed until forward_token is called.
    let preproc = unsafe {
        core::slice::from_raw_parts_mut(
            (slot.inf_buf_addr as *mut u8).add(slot.inf_logits * 4),
            MAX_PREPROC_INPUT,
        )
    };
...
fn generate_from_tokens(
    ...
    // Debug: print logits after prefill (top-3)
    {
        let logits_ptr = (slot.inf_buf_addr as *const i32).wrapping_add(slot.inf_logits);
...
fn agent_loop_local(prompt: &[u8]) -> Result<KVVec<u8>> {
    ...
    // Forward token 1 (<|im_start|>) at position 0 and check top prediction
    {
        ...
        pr_info!("hackbot: DEBUG single-token test: after token 1, top1 = {} (logit {})\n",
                 top1, top1_logit);
        // Expected: top1 should be 28 (',') with logit ~1304762 (from Python reference)
        // Also check: what's the logit for token 28?
