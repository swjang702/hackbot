...
///
/// `busy` enforces single-writer semantics: `write_iter` sets it on entry,
/// runs `process_prompt` outside the lock, then clears it on exit. This
/// prevents a last-writer-wins race where two concurrent `write_iter`
/// callers interleave their RESPONSE updates and a reader observes a
/// payload from one writer with the `len`/`ready` flags from the other.
/// See R-018 in docs/REVIEW_v0.1.md.
